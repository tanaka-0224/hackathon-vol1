...
const Game = () => {
    ...
    const [tips, setTips] = useState(""); // 提示信息状态
    ...
    useEffect(() => {
        const fetchTips = async () => {
            try {
                const response = await fetch("/gpt/scene_chat");
                if (!response.ok) {
                    throw new Error("Network response was not ok");
                }
                const data = await response.json();
                console.log(data);
                setTips(data.hint_message);
            } catch (error) {
                console.error("Error fetching tips:", error);
                setTips(""); // 如果获取失败，将提示信息清空或设置为默认值
            }
        };

        fetchTips();
    }, [selectedSlang]); // 仅当 selectedSlang 改变时执行
    ...
    const handleSlangClick = async (slang, rowIndex, columnIndex) => {
        ...
        try {
            const response = await fetch(GPT_CHAT_URI, {
                ...
                body: JSON.stringify({ input_sentences: slang }), // 发送当前单词到 API
            });
            ...
            // 更新 UI 显示相关信息
            setTips(data.chat_message);
        } catch (error) {
            console.error('Error fetching tips:', error);
            // 如果获取失败，将提示信息设置为空字符串或其他默认值
            setTips('');
        }
